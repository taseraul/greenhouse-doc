...
function populateDevice(device) {
  var devicePath = 'devices/' + device.toString() + '/properties';
  ...
  deviceRef.get().then((snapshot) => {
    if (snapshot.exists()) {
      ...
      var relay = document.createElement("p");
      ...
      chartContainer.id = 'container' + device.toString();
      ...
      relay.innerHTML = "Relay state : " + snapshot.val().relay.state;

      var dataPath = 'devices/' + device.toString() + '/data';
      ...
      dataRef.limitToLast(20).orderByChild("timestamp").on("child_added", data => {
        ...
        var jsonData = data.toJSON(); // example: {temperature: 25.02, humidity: 50.20, pressure: 1008.48, timestamp:1641317355}
        ...
        // for (var key in logPoint) {
        //   console.log(key);
        //   console.log(logPoint[key]);
        battery.innerHTML = "Battery : " + jsonData.battery;
        temp.innerHTML = "Temp : " + jsonData.temperature + '*C';
        // }
      });


    } else {
      console.log("No device details");
...
const setupUI = (user) => {
  if (user) {
    ...
    var userPath = 'users/' + uid.toString();
    ...
    userRef.get().then((snapshot) => {
      if (snapshot.exists()) {
        ...
        snapshot.val().devices.forEach(element => {
          populateDevice(element);
        });
      } else {
        console.log("No devices associated");
      }
    }).catch((error) => {
      console.error(error);
    });
